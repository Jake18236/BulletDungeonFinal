...
export function PauseScreen() {
  const { phase, pause, resume, restart } = useGame();
  const { isMuted, toggleMute, masterVolume, setVolume } = useAudio();
  const [pauseMenuScreen, setPauseMenuScreen] = useState<"main" | "options">("main");
  ...
  useEffect(() => {
    ...
    drawBitmapText(ctx, "PAUSED", canvas.width / 2, canvas.height / 2, font, fontRedImage, {
      align: "center",
      baseline: "middle",
      scale: 4,
    });
  ...
  return (
    <div className="fixed inset-0 z-[25] font-pixel pointer-events-none">
      <div
      ...
      <div
        className="absolute left-1/2 top-1/2 w-full max-w-md px-4 pointer-events-auto"
        style={{
          transform: "translate(-50%, -50%)",
        }}
      >
        <div className="menu-shell-glow absolute inset-0 pointer-events-none" />
        ...
        {pauseMenuScreen === "main" && (
          <div className="space-y-4">
            <Button onClick={handleResume} className="menu-btn w-full text-lg py-6 menu-font">
              Resume
            </Button>
            <Button onClick={() => setPauseMenuScreen("options")} className="menu-btn w-full text-lg py-6 menu-font">
              Options
            </Button>
            <Button onClick={handleGiveUp} className="menu-btn w-full text-lg py-6 menu-font" style={{
              background: "linear-gradient(90deg, rgba(180, 20, 20, 0.85), rgba(160, 10, 10, 0.65))",
            }}>
              Give Up
            </Button>
          </div>
        ...
        {pauseMenuScreen === "options" && (
          <div className="space-y-6">
            ...
            <Button onClick={() => setPauseMenuScreen("main")} className="menu-btn w-full mt-2 menu-font">
              Back
            </Button>
          </div>
        )}
      ...
      <style>{`
        .menu-shell-glow {
          background: radial-gradient(circle at center, rgba(185, 28, 28, 0.15), rgba(0, 0, 0, 0.0) 70%);
        }
        .menu-font {
